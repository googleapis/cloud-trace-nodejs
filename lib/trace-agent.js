...
TraceAgent.prototype.createRootSpanData = function(name, traceId, parentId) {
  ...
  return new OpaqueSpan(spanData);
};
 ...
 * Google services.
 *
 * @param {OpaqueSpan} opaque The span to be added to headers.
 * @param {Object} headers The http headers associated with the
 *   current request.
 */
TraceAgent.prototype.addContextToHeaders = function(opaque, headers) {
  if (!opaque.spanData_) {
    return;
  }
  var spanData = opaque.spanData_;
