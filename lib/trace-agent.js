...
/** @const {string} a header that identifies requests that we make ourself. */
TraceAgent.prototype.TRACE_API_HEADER_NAME = 'X-Cloud-Trace-Agent';

/** @const {string} a header that is used to identify trace across API calls. */
TraceAgent.prototype.TRACE_CONTEXT_HEADER_NAME = 'X-Cloud-Trace-Context';
...
TraceAgent.prototype.isTraceAPIRequest = function(options) {
  return options && options.headers &&
    !!options.headers[this.TRACE_API_HEADER_NAME];
};
...
module.exports = {
  get: function(config) {
    ...
    traceAgent = new TraceAgent(config);
    return traceAgent;
  }
};
