...
/**
 * Wraps the gRPC server register function in order to create trace spans for
 * gRPC service methods.
 * @param {Object} server The server module.
 */
function wrapServer(server) {
  var unwrapFnList = [];
  function serverRegisterWrap(original) {
    return function registerTrace(name, handler, serialize, deserialize, method_type) {
      ...
      if (method_type === 'unary') {
        ...
        agent.logger.warn('gRPC Server: Unrecognized method_type ' +
          method_type);
      }
      unwrapFnList.push(function() {
        if (handlerSet.deserialize.__wrapped) {
          shimmer.unwrap(handlerSet, 'deserialize');
        }
        if (handlerSet.func.__wrapped) {
          shimmer.unwrap(handlerSet, 'func');
        }
        if (handlerSet.serialize.__wrapped) {
          shimmer.unwrap(handlerSet, 'serialize');
        }
      });
      return result;
    };
  }
  var wrapped = shimmer.wrap(server.Server.prototype, 'register',
    serverRegisterWrap);
  wrapped.__unwrap = function() {
    unwrapFnList.forEach(function(fn) { fn(); });
  };
}

/**
 * Unwraps the gRPC server register function and any functions that were
 * wrapped earlier.
 * @param {Object} server The server module.
 */
function unwrapServer(server) {
  if (server.Server.prototype.register.__original) {
    // need to unwrap
    if (server.Server.__unwrap) {
      server.Server.__unwrap.forEach(function(fn) { fn(); });
      server.Server.__unwrap = [];
    }
