...
function wrapUnary(namespace, handlerSet, requestName) {
  ...
  shimmer.wrap(handlerSet, 'deserialize', function (deserialize) {
    return namespace.bind(function deserializeTrace() {
      ...
      } else if (handlerSet.deserialize.__wrapped) {
  ...
  shimmer.wrap(handlerSet, 'func', function (func) {
    return namespace.bind(function funcTrace(stream, callback) {
      if (agent.isActive()) {
        rootContext = startRootSpanForRequest(requestName);
        if (agent.config_.enhancedDatabaseReporting) {
          shimmer.wrap(stream, 'sendMetadata',
            sendMetadataWrapper(rootContext));
        }
        ...
        arguments[1] = function (err, result, trailer, flags) {
          if (agent.config_.enhancedDatabaseReporting) {
            ...
            if (trailer) {
              rootContext.addLabel('trailing_metadata',
      ...
      } else if (handlerSet.func.__wrapped) {
        shimmer.unwrap(handlerSet, 'func');
        shimmer.unwrap(handlerSet, 'serialize');
      }
      return func.apply(this, arguments);
...
function wrapServer(server) {
  ...
  shimmer.wrap(server.Server.prototype, 'register',
    serverRegisterWrap);
}

/**
 * Unwraps the gRPC server register function.
 * @param {Object} server The server module.
 */
function unwrapServer(server) {
  shimmer.unwrap(server.Server.prototype, 'register');
  // Note that this doesn't unwrap the handlers for each service method.
  // TODO: Lazily unwrap the handlers when they're invoked.
}
