...
function nextWrap(next) {
  return function next_trace(cb) {
    ...
    var spanData = root.createChildSpanData('mongo-cursor');
    spanData.span.setLabel('db', this.ns);
    return next.call(this, wrapCallback(spanData, cb));
  };
}
...
function wrapWithLabel(label) {
  return function(original) {
    return function mongo_operation_trace(ns, ops, options, callback) {
      ...
      var currContext = root.createChildSpanData(label);
      currContext.span.setLabel('db', ns);
      if (typeof options === 'function') {
        return original.call(this, ns, ops,
          wrapCallback(currContext, options));
      } else {
        return original.call(this, ns, ops, options,
          wrapCallback(currContext, callback));
      }
    };
  };
}

function wrapCallback(spanData, done) {
