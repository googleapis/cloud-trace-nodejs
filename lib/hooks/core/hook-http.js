...
function requestWrap(request) {
  return function request_trace(options, callback) {
    ...
    var currContext = cls.getRootContext().createChildSpanData(uri);
    currContext.span.setLabel(TraceLabels.HTTP_METHOD_LABEL_KEY, options.method);
    currContext.span.setLabel(TraceLabels.HTTP_URL_LABEL_KEY, uri);
    if (options.headers) {
      // Adding context to the headers lets us trace the request
      // as it makes it through other layers of the Google infrastructure
      currContext.addContextToHeaders(options.headers);
    }
    ...
    var returned = request.call(this, options, function(res) {
      ...
      res.on('end', function() {
        currContext.span.setLabel(
          TraceLabels.HTTP_RESPONSE_SIZE_LABEL_KEY, numBytes);
        currContext.span.setLabel(
