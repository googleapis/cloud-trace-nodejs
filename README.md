...
* Requests originating from the body of a `then` implementation in a userspace thenable may not get traced if the thenable is `await`-ed. This is largely an unconventional case but is present in the `knex` module, which monkeypatches the Bluebird Promise's prototype to make database calls. __If you are using `knex` (esp. the `raw` function), see [https://github.com/googleapis/cloud-trace-nodejs/issues/946](#946) for more details on whether you are affected, as well as a suggested workaround.__
  * This is the result of a Node issue that is currently being tracked in [https://github.com/nodejs/node/issues/26064](nodejs/node#26064).
