...
describeInterop<typeof knexTypes>('knex', (fixture) => {
  ...
  before(() => {
    ...
    knex = fixture.require()(
        {client: 'mysql', connection: require('../mysql-config')});
  ...
  beforeEach(() => {
    return knex.schema
        ...
        .then((result) => {
          assert.ok(result);
          return knex.insert(obj).into(TABLE_NAME).then((result) => {
            assert.ok(result);
  ...
  it('should perform basic operations using ' + version, () => {
    return tracer.runInRootSpan({name: 'outer'}, (rootSpan) => {
      return knex(TABLE_NAME).select().then((res) => {
        rootSpan.endSpan();
        assert(res);
  ...
  it('should propagate context using ' + version, () => {
    return tracer.runInRootSpan({name: 'outer'}, (rootSpan) => {
      return knex.select().from(TABLE_NAME).then((res) => {
  ...
  it('should remove trace frames from stack using ' + version, () => {
    return tracer.runInRootSpan({name: 'outer'}, (rootSpan) => {
      return knex.select().from(TABLE_NAME).then((res) => {
        ...
        const stackTrace =
            JSON.parse(labels[TraceLabels.STACK_TRACE_DETAILS_KEY]);
  ...
  it('should work without events or callback using ' + version, () => {
    return tracer.runInRootSpan({name: 'outer'}, (rootSpan) => {
      return knex.select().from(TABLE_NAME).then(async (result) => {
        await wait(50);
        rootSpan.endSpan();
        const spans = traceTestModule.getSpans((span) => {
          return span.name === 'mysql-query';
