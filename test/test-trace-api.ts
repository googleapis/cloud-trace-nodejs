...
describe('Trace Interface', () => {
  ...
  describe('behavior when initialized', () => {
    ...
    it('should respect filter methods', () => {
      ...
      traceAPI.runInRootSpan(
        { name: 'root', method: 'alternativeMethod' },
        rootSpan => {
          assert.strictEqual(rootSpan.type, SpanType.ROOT);
        }
      );
    ...
    it('should respect contextHeaderBehavior options field', () => {
      ...
      createTraceAgent({
        ...
        { name: 'root1', traceContext: '123456/667;o=1' },
        rootSpan => {
          rootSpan.endSpan();
        }
      );
      // The trace ID will not randomly be 123456
      let foundTrace = testTraceModule.getOneTrace(
      ...
      createTraceAgent({
        ...
        { name: 'root2', traceContext: '123456/667;o=1' },
        rootSpan => {
          rootSpan.endSpan();
        }
      );
      foundTrace = testTraceModule.getOneTrace(
        trace => trace.traceId === '123456'
      );
      assert.strictEqual(foundTrace.spans.length, 1);
      ...
      assert.strictEqual(
        testTraceModule.getSpans(span => span.name === 'root3').length,
        0
      );
    });
    ...
    it('should trace if no option flags are provided', () => {
      createTraceAgent({ enhancedDatabaseReporting: false }).runInRootSpan(
        { name: 'root', traceContext: '123456/667' },
        rootSpan => {
          rootSpan.endSpan();
        }
      );
